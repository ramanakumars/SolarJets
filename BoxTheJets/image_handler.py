...
def world_from_pixel(metadata, x, y):
    '''
    ...
    ----------
    metadata : dict
        The metadata for this Zooniverse subject
        Used to reconstruct image coordinates as real world coordinates
    x : float
    ...
    if x > 1 and y > 1:
        x = x / float(metadata['#width'])
        y = y / float(metadata['#height'])

    #Convert metadata keys into 'proper' keys readable by sunpy
    fits_headers = {
