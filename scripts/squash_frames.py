...
def merge_tasks(data, task1='T1', task2='T3'):
    ...
    for k, col0k in enumerate(col0):
        ...
        colT5 = col0k.replace(task1, task2)
        ...
        for row in mask:
            ...
            row_T1 = np.where((data_merged['classification_id'][:] == classification_id) & (data_merged['task'][:] == task1))[0][0]
            try:
                dataT1 = json.loads(data_merged[col0k][row_T1])
            except ValueError:
                dataT1 = []

            dataT5 = json.loads(data_merged[colT5][row])
